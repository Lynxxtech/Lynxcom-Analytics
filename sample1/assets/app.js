...
function render(){
 const k=DATA.kpis;
 const cards=[['Active Students',k.active_students,'current records'],['Primary Students',k.primary_students,'day section'],['Boarders',k.secondary_boarders,'secondary section'],['Fee Collection',k.collection_rate+'%','paid vs billed'],['Outstanding Fees',shortMoney(k.outstanding_fees),'requires action'],['Parent Debtors',k.debtors_count,'follow-up list']];
 document.getElementById('kpis').innerHTML=cards.map(c=>`<article class="metric-card"><span>${c[0]}</span><strong>${c[1]}</strong><small>${c[2]}</small></article>`).join('');
 ...
 chart('feeStatusChart','bar',['Billed','Collected','Outstanding'],[{label:'Amount',data:[k.fees_billed,k.fees_collected,k.outstanding_fees],backgroundColor:['#061b31','#079455','#d92d20'],borderRadius:10}],{indexAxis:'y',scales:{x:{ticks:{callback:v=>shortMoney(v)}}}});
...
function renderDebtors(){
 ...
 const filtered=debtors.filter(d=>(!cls||d.class===cls)&&(!pri||d.priority===pri)&&(!q||(`${d.student_name} ${d.guardian_name} ${d.guardian_phone} ${d.class}`).toLowerCase().includes(q)));
 const rows=filtered.slice(0,100);
 document.getElementById('visibleDebtors').textContent=filtered.length;
 document.getElementById('totalDebt').textContent=money(filtered.reduce((s,d)=>s+Number(d.outstanding_balance||0),0));
